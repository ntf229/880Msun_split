...
# Find best position for 50 kpc cube by considering all possibilities 
# Select the one which contains the most stellar mass
#xRange = [np.amin(full_x_pos), np.amax(full_x_pos)]
#yRange = [np.amin(full_y_pos), np.amax(full_y_pos)]
#zRange = [np.amin(full_z_pos), np.amax(full_z_pos)]
boxSize = 5.e4 # in pc
xRange = [xAbsMin - 2*boxSize, xAbsMax + 2*boxSize]
yRange = [yAbsMin - 2*boxSize, yAbsMax + 2*boxSize]
zRange = [zAbsMin - 2*boxSize, zAbsMax + 2*boxSize]
stepSize = 5.e3 # in pc
numSteps = int(np.amax(np.asarray([xRange[1]-xRange[0],yRange[1]-yRange[0],zRange[1]-zRange[0]]))/stepSize)
print('number of steps for box location:',numSteps)
bestIndex = [0,0,0]
maxMass = 0.
for x in range(numSteps): 
    for y in range(numSteps): 
        for z in range(numSteps): 
            xMin = xRange[0]+(x*stepSize)
            xMax = xMin + stepSize
            yMin = yRange[0]+(y*stepSize)
            yMax = yMin + stepSize
            zMin = zRange[0]+(z*stepSize)
            zMax = zMin + stepSize
            xMaskMin = full_x_pos > xMin
            xMaskMax = full_x_pos[xMaskMin] < xMax
            yMaskMin = full_y_pos[xMaskMin][xMaskMax] > yMin 
            yMaskMax = full_y_pos[xMaskMin][xMaskMax][yMaskMin] < yMax
            zMaskMin = full_z_pos[xMaskMin][xMaskMax][yMaskMin][yMaskMax] > zMin
            zMaskMax = full_z_pos[xMaskMin][xMaskMax][yMaskMin][yMaskMax][zMaskMin] < zMax
            encMass = np.sum(full_mass[xMaskMin][xMaskMax][yMaskMin][yMaskMax][zMaskMin][zMaskMax]) # total stellar mass within box
            if encMass > maxMass:
                maxMass = encMass
                bestIndex = [x,y,z]

print('spatial box index:', bestIndex)
print('stellar mass enclosed:', maxMass)

# Set spatial cut region (in pc)   
xAbsMin = xRange[0]+(bestIndex[0]*stepSize)
xAbsMax = xMin + stepSize
yAbsMin = yRange[0]+(bestIndex[1]*stepSize)
yAbsMax = yMin + stepSize
zAbsMin = zRange[0]+(bestIndex[2]*stepSize)
zAbsMax = zMin + stepSize
