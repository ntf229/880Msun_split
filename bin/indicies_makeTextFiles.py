...
# Load spatial cut indicies from snapshot 696 (selected by hand)
starIndex = np.load('/scratch/ntf229/880Msun_split/resources/particles/starIndex.npy')
#gasIndex = np.load('/scratch/ntf229/880Msun_split/resources/particles/gasIndex.npy')
...
center696 = np.load('/scratch/ntf229/880Msun_split/resources/particles/center.npy')
center = [np.average(cut_x_pos, weights=cut_mass), np.average(cut_y_pos, weights=cut_mass), np.average(cut_z_pos, weights=cut_mass)]

x_offset = cetner696[0] - center[0]
y_offset = cetner696[1] - center[1]
z_offset = cetner696[2] - center[2]

# shift all star particles to have the same center of mass as snapshot 696
cut_x_pos += x_offset
cut_y_pos += y_offset
cut_z_pos += z_offset

if index == 0: # parent snapshot
    # make spatial cuts by considering box size and cetner
    boxSize = 5.e4 # box side length in pc
    full_x_pos_gas += x_offset
    full_y_pos_gas += y_offset
    full_z_pos_gas += z_offset
    xMin = full_x_pos > -1*boxSize/2
    xMax = full_x_pos[xMin] < 1*boxSize/2
    yMin = full_y_pos[xMin][xMax] > -1*boxSize/2
    yMax = full_y_pos[xMin][xMax][yMin] < 1*boxSize/2
    zMin = full_z_pos[xMin][xMax][yMin][yMax] > -1*boxSize/2
    zMax = full_z_pos[xMin][xMax][yMin][yMax][zMin] < 1*boxSize/2
    cut_x_pos_gas = full_x_pos_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_y_pos_gas = full_y_pos_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_z_pos_gas = full_z_pos_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_smooth_gas = full_smooth_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_mass_gas = full_mass_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_metals_gas = full_metals_gas[xMin][xMax][yMin][yMax][zMin][zMax]
    cut_temp_gas = full_temp_gas[xMin][xMax][yMin][yMax][zMin][zMax]
else:
    gasIndex = np.load('/scratch/ntf229/880Msun_split/resources/particles/gasIndex.npy')
    ...
    # shift all gas particles to have the same center of mass as snapshot 696
    cut_x_pos_gas = cut_x_pos_gas + x_offset
    cut_y_pos_gas = cut_y_pos_gas + y_offset
    cut_z_pos_gas = cut_z_pos_gas + z_offset
